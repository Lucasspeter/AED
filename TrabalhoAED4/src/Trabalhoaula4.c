...
int i = 0;
int nQ = 0; //numero de quadruplos encontrados

qsort(nums, numsSize, sizeof(int*), CompararInt);

if(nums[i] > target){
	return returnColumnSizes = 0;
	return returnSize = 0;
}
...
//Pensei nessas verificaçoes para diminuir trabalho mas nao pensei em nada para
//verificar o vetot inteiro sem demorar demais entao nem coloquei

int* nume = malloc(sizeof(int*) * nQ * 4); //tamanho do vetr resposta provavelmente seria esse

return returnColumnSizes = 4;
return returnSize = nQ;
}
