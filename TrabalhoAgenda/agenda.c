...
void adicionarPessoa( void **pBuffer ) {
    ...
    // 1. Definir onde é o rascunho (pula os 3 ints de controle)
    ...
    // 3. Calcular o tamanho que esta nova pessoa ocupará
    ...
    *pNovoBlocoTam = ( sizeof ( int ) * 2 ) + ( strlen ( pRascunhoNome ) + 1 ) + ( strlen( pRascunhoEmail ) + 1 );
    ...
    // 4. Atualizar o tamanho total e dar o Realloc
    pAntigoTamTotal = ( int* )( *pBuffer ) + 2; // Ponteiro para o "Tamanho Total" no cabeçalho
    ...
    *pBuffer = realloc( *pBuffer, *pAntigoTamTotal + *pNovoBlocoTam );
    ...
    // 5. Copiar os dados para o novo espaço alocado
    // recalculando o ponto de inserção usando o novo endereço de *pBuffer
    pNovoBlocoTam = ( int* ) ( ( char* ) ( *pBuffer ) + ( * ( ( int* ) ( *pBuffer ) + 2 ) ) ); // Usa o tamanho guardado na posição +2
    
    // [int tam_bloco]
    *pNovoBlocoTam = ( sizeof ( int ) * 2 ) + ( strlen ( pRascunhoNome ) + 1 ) + ( strlen ( pRascunhoEmail ) + 1 );
    
    // [int idade] - Lendo do novo endereço correto do rascunho
    * ( pNovoBlocoTam + 1 ) = *pRascunhoIdade; 
...
void listarTodos( void *pBuffer ) {
    ...
    pContadorPessoas = ( int* ) pBuffer + 1;
...
void buscarPessoa( void *pBuffer ) {
    ...
    // 1. Onde está o termo de busca? (Início do rascunho)
    pBusca = ( char* ) pBuffer + ( 3 * sizeof ( int ) ); // O rascunho começa aqui

    // 2. Onde guardamos se encontramos? (No final do rascunho, byte 200)
    ...
    *pFlagEncontrado = 0; // Começa como 0 (não encontrado)
    ...
    // 3. Preparar a caminhada (igual ao listar)
    pAux = ( char* ) pBuffer + ( 3 * sizeof ( int ) ) + 256;
...
void removerPessoa( void **pBuffer ) {
    ...
    // 1. Configurar ponteiros de controle no rascunho
    pBusca = ( char* ) ( *pBuffer ) + ( 3 * sizeof ( int ) ); // Início do rascunho
    pFlagEncontrado = (int*)(pBusca + 200);         // Status no final do rascunho
    *pFlagEncontrado = 0;
    ...
    // 2. Iniciar busca (igual a função buscar)
    pAux = ( char* )(*pBuffer) + ( 3 * sizeof ( int ) ) + 256;
    pLimite = ( char* )(*pBuffer) + *( ( int* ) (*pBuffer) + 2 );
    ...
    while ( pAux < pLimite ) {
        ...
        if ( strcmp ( pBusca, pAux + ( sizeof ( int ) * 2 ) ) == 0 ) {
            *pFlagEncontrado = 1;
            ...
            // A. Guardar o tamanho desta pessoa no rascunho (para subtrair depois)
            // Usamos o rascunho + 210 como "váriavel temporária"
            *( ( int* ) ( pBusca + 210 ) ) = *( ( int* ) pAux );

            // B. Deslocar a memória (memmove)
            ...
            // C. Atualizar cabeçalho global
            *( ( int* ) ( *pBuffer ) + 1 ) -= 1; // Menos 1 pessoa
            *( ( int* ) ( *pBuffer ) + 2 ) -= *( ( int* ) ( pBusca + 210 ) ); // Diminui tamanho total

            // D. Encolher o buffer com realloc
            *pBuffer = realloc( *pBuffer, *( ( int* ) ( *pBuffer ) + 2 ) );
